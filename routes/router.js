...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, path.join(__dirname, '../../client/public/Assets')); // Store images in the 'public/images' folder
  },
  filename: (req, file, cb) => {
    cb(null, `${Date.now()}-${file.originalname}`); // Save files with a timestamp and original name
  },
});
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'User registered successfully', userType:"user" });
  } catch (err) {
    console.error(err);
...
router.post('/generate-description', async (req, res) => {
  ...
  const prompt = `Generate a concise and engaging product title (20 words max) for a product named "${productName}" in the "${category}" category. Ensure that the title does not contain # or *.`;

  try {
    const result = await model.generateContent(prompt);
    let generatedTitle = result.response.text();

    // Limit to 10 words and sanitize the title
    ...
    res.json({ description: sanitizedTitle });
  } catch (error) {
...
router.post('/profile', async (req, res) => {
  ...
  try {
    const shopkeeper = await Shopkeeper.findOne({ email }).select('-password'); // Exclude password
    if (!shopkeeper) {
...
router.post('/profileuser', async (req, res) => {
  ...
  try {
    const shopkeeper = await User.findOne({ email }).select('-password'); // Exclude password
    if (!shopkeeper) {
      return res.status(404).json({ error: 'Shopkeeper not found' });
    }
